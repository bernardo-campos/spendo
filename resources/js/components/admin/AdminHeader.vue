...
<template>
    <header class="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur">
        <div class="flex h-16 items-center gap-3 px-4 lg:px-6">
            ...
            <label for="navbar-period" class="flex min-w-0 flex-1 items-center gap-2 text-sm font-medium sm:max-w-64">
                <CalendarDays class="hidden size-4 shrink-0 text-muted-foreground sm:block" />
        ...
        <button v-if="syncState.status === 'offline'" type="button" class="flex w-full items-center justify-center border-t border-border bg-accent px-4 py-2 text-xs font-medium text-muted-foreground sm:hidden" @click="emit('retry-sync')">
            {{ syncLabel(syncState) }}
        </button>
    </header>
</template>
